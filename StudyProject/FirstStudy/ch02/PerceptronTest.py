import numpy as np

def AND_v1(x1, x2):
    w1, w2, theta = 0.5, 0.5, 0.7
    tmp = x1 * w1 + x2 * w2
    if tmp <= theta:
        return 0
    elif tmp > theta:
        return 1

print('AND_v1(0, 0) : ' + str(AND_v1(0, 0)))
print('AND_v1(0, 1) : ' + str(AND_v1(0, 1)))
print('AND_v1(1, 0) : ' + str(AND_v1(1, 0)))
print('AND_v1(1, 1) : ' + str(AND_v1(1, 1)))

def AND_v2(x1, x2):
    x = np.array([x1, x2])
    w = np.array([0.5, 0.5])
    b = -0.7
    tmp = np.sum(x * w) + b
    if tmp <= 0:
        return 0
    else:
        return 1

print('AND_v2(0, 0) : ' + str(AND_v2(0, 0)))
print('AND_v2(0, 1) : ' + str(AND_v2(0, 1)))
print('AND_v2(1, 0) : ' + str(AND_v2(1, 0)))
print('AND_v2(1, 1) : ' + str(AND_v2(1, 1)))

def NAND(x1, x2):
    x = np.array([x1, x2])
    w = np.array([-0.5, -0.5])
    b = 0.7
    tmp = np.sum(x * w) + b
    if tmp <= 0:
        return 0
    else:
        return 1

print('NAND(0, 0) : ' + str(NAND(0, 0)))
print('NAND(0, 1) : ' + str(NAND(0, 1)))
print('NAND(1, 0) : ' + str(NAND(1, 0)))
print('NAND(1, 1) : ' + str(NAND(1, 1)))

def OR(x1, x2):
    x = np.array([x1, x2])
    w = np.array([0.5, 0.5])
    b = -0.2
    tmp = np.sum(x * w) + b
    if tmp <= 0:
        return 0
    else:
        return 1

print('OR(0, 0) : ' + str(OR(0, 0)))
print('OR(0, 1) : ' + str(OR(0, 1)))
print('OR(1, 0) : ' + str(OR(1, 0)))
print('OR(1, 1) : ' + str(OR(1, 1)))

def XOR(x1, x2):
    s1 = NAND(x1, x2)
    s2 = OR(x1, x2)
    y = AND_v2(s1, s2)
    return y

print('XOR(0, 0) : ' + str(XOR(0, 0)))
print('XOR(0, 1) : ' + str(XOR(0, 1)))
print('XOR(1, 0) : ' + str(XOR(1, 0)))
print('XOR(1, 1) : ' + str(XOR(1, 1)))